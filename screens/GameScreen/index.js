...
import { StyleSheet, View, Text, ScrollView, InteractionManager } from 'react-native';
...
const GameScreen = ({ navigation, route }) => {
	...
	getGameDetails = async (objectId) => {
		...
		console.log(`getGameDetails: ${JSON.stringify(item)}`);
		setDetails(item);
	};
	...
	getGameStats = async (objectId) => {
		...
		console.log(`getGameStats: ${JSON.stringify(newItemStats)}`);
		setItemStates(newItemStats);
	};

	_renderHeaderRank = () => {
		let { item: { rankinfo: rankInfo } } = itemStats;
			...
			return (
				<View style={styles.headerRatings}>
					...
					{rankInfo.map((rank, i) => (
						...
						>
							{rank.veryshortprettyname.toUpperCase().trim()}: {rank.rank.toUpperCase()}
						</Text>
					))}
				</View>
			);
	...
	_renderHeaderName = () => {
		const { item: { stats: stats = { average: '0' } } } = itemStats;
		...
		return (
			<View style={{ flexDirection: 'row' }}>
				<View style={[ styles.headerIcon, { backgroundColor: ratingBGColor } ]}>
					<Text style={{ color: '#ffffff', fontSize: 24, fontFamily: 'lato-bold' }}>{ratingText}</Text>
				</View>
				...
				<View style={{ flex: 1, flexDirection: 'row', flexWrap: 'wrap' }}>
					...
					>
						{game.name}
						{details ? <Text style={{ color: '#dee0e2' }}> ({details.yearpublished})</Text> : null}
					</Text>
					<Text
						style={{
							width: '100%',
	...
	_renderGameStats = (details) => {
		const { item: { polls: polls } } = itemStats;
		...
		if (polls !== undefined && details !== null) {
			return (
				<View style={{ flex: 1, flexDirection: 'row', flexWrap: 'wrap' }}>
					...
					<View style={[ styles.statsBox, styles.statsBoxRight, styles.statsBoxTop ]}>
					...
					<View style={[ styles.statsBox, styles.statsBoxTop ]}>
						<Text style={styles.statsTitle}>
							Weight: {_trimTo(polls.boardgameweight.averageweight, 2)} / 5
						</Text>
		...
		if (images.previewthumb) {
			...
			return (
				<View style={styles.emptyView}>
					<ProgressBar indeterminate={true} color="#ffffff" style={{ margin: 12 }} />
	...
	return (
		<ScrollView>
			<View style={styles.itemContainer}>
				...
				{_renderHeaderRank()}
				<View style={{ padding: 10, backgroundColor: '#000000' }}>{_renderHeaderName(route.params)}</View>
				<View style={{ padding: 10, backgroundColor: '#E7ECF1' }}>
					{_renderGameStats(details)}
					{_renderCredits(details)}
				</View>
				<View style={styles.headerBottonRow}>
					<RateButton navigation={navigation} game={game} />
