...
export const fetchCollectionFromBGG = async (username) => {
	...
	const url = `https://www.boardgamegeek.com/xmlapi2/collection?username=${username}&stats=1`;
	...
	try {
		...
		if (response.status == 202) {
			...
			for (item of doc.getElementsByTagName('item')) {
				...
				// extract user's game rating
				const statsElement = item.getElementsByTagName('rating');
				const ratingElement = statsElement.getElementsByTagName('rating');
				const userRating = ratingElement.attributes.value;

				const numPlays = getElementValue(item, 'numplays');
			...
			Sentry.captureMessage('Non 200/202 Response from BGG when loading collection.', 'error');
		}
	} catch (error) {
